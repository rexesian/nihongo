...
ktbjx <- read_csv("rdata/Sheet 142-KNJRNMX.csv")
vxsnt <- read_csv("rdata/Sheet 143-VXDFG.csv")
ktbs <- read_csv("rdata/Sheet 144-KNJRNMX.csv")
ctad <- read_csv("rdata/Sheet 147-CTADM.csv")
blgmr <- read_csv("rdata/Sheet 148-SSTNC.csv")
ilgmr <- read_csv("rdata/Sheet 149-SSTNC.csv")
algmr <- read_csv("rdata/Sheet 150-SSTNC.csv")
blgnn <- read_csv("rdata/Sheet 151-SSTNC.csv")
cmpdv <- read_csv("rdata/Sheet 154-CMPMD.csv")
wrdmm <- read_csv("rdata/Sheet 155-TBL.csv")
...
ktbjn1 <- ktbjn %>% slice(1:80)
ktbjn1 <- ktbjn1[sample(1:nrow(na.omit(ktbjn1))),]
  ...
  columns = list(
    Kanji = colDef(
      cell = function(value, index) {
        ...
        Base.Meaning <- ktbjn1$Base.Meaning[index]
...
ktbjn2 <- ktbjn %>% slice(81:110)
ktbjn2 <- ktbjn2[sample(1:nrow(na.omit(ktbjn2))),]
  ...
  columns = list(
    Kanji = colDef(
      cell = function(value, index) {
        ...
        w5 <- ktbjn2$w5[index]
        w5 <- if (!is.na(w5)) w5 else "."
        Base.Meaning <- ktbjn2$Base.Meaning[index]
        Base.Meaning <- if (!is.na(Base.Meaning)) Base.Meaning else "."
        tagList(
          div(style = list(fontSize = 13, fontWeight = 500), w1, "・", w2, "・", w3, "・", w4, "・", w5),
          div(style = list(fontSize = 19, fontWeight = 500), value),
...
```{r, echo=FALSE, message=FALSE}
set.seed(7634)
ktbjx <- ktbjx[sample(1:nrow(na.omit(ktbjx))),]
reactable(
  na.omit(ktbjx),
  columns = list(
    Words = colDef(
      cell = function(value, index) {
        ...
        w5 <- if (!is.na(w5)) w5 else "."
        tagList(
          div(style = list(fontSize = 19, fontWeight = 500), value, "・", w2, "・", w3, "・", w4, "・", w5),
        )
      },
      align = "center"
    ),
...
ktbs1 <- ktbs %>% slice(1:80)
set.seed(7634)
ktbs1 <- ktbs1[sample(1:nrow(na.omit(ktbs1))),]
reactable(
  na.omit(ktbs1),
  columns = list(
    Words = colDef(
...
reactable(
  ...
  columns = list(
    Sentences = colDef(
      # Show species under character names
      cell = function(value, index) {
        Translation <- vxsnt$Translation[index]
        Translation <- if (!is.na(Translation)) Translation else "Unknown"
        tagList(
          div(style = list(fontSize = 19, fontWeight = 500), value),
          div(style = list(fontSize = 11), Translation)
    ...
    Meaning = colDef(
      cell = function(value, index) {
        ...
        tagList(
          ...
          div(style = list(fontSize = 11, fontWeight = 200), Verbs)
        )
      },
      align = "center"
    ),
  ...
  columns = list(
    Meaning = colDef(
      cell = function(value, index) {
        ...
        Verbs <- if (!is.na(Verbs)) Verbs else "Unknown"
        tagList(
          div(style = list(fontSize = 25, fontWeight = 600), Verbs),
          # div(style = list(fontSize = 11, fontWeight = 200), value),
        )
      },
      align = "center"
    ),
...
**沿う** そう `to run along; to run beside; to stick to (a line); to follow (a plan, etc.); to act in accordance with`  
**浮く** うく `to float; to become merry; to become loose; to become unsteady; to feel out of it; to be cut off`  
...
**注ぐ** そそぐ `to pour (into); to sprinkle; to water to pour onto; to spray; to flow into; to drain into; to devote to`  
**抜く** ぬく `to pull out; to draw out; to extract; to unplug; to omit; to leave out; to carry through; to let out`  
**拭く** ふく `to wipe; to dry`  
**押す** おす `to push; to press; to apply pressure from above; to press down; to stamp; to affix; to press`  
**射す** さす `to shine`  
**撃つ** うつ `to shoot (at); to attack; to defeat; to destroy; to avenge`  
**持つ** もつ `to hold (in one's hand); to take; to carry; to possess; to have; to own; to maintain; to keep`  
**酌む** くむ `to pour (sake); to serve; to drink together`  
**揉む** もむ `to rub; to crumple; to massage; to argue; to have a heated discussion; to train; to coach; to jostle`  
**入る** はいる `to enter; to go into; to break into; to join; to enroll`  
**酔う** よう `to get drunk; to become intoxicated; to feel sick; to become nauseated; to be elated; to be exalted`  

------------------------------------------------------------------------

**会う** あう `to meet; to encounter; to see`

    彼女には全然会ったことがない　　　　　　　　I've never met her before
    彼には会わないようにしなさい　　　　　　　　Avoid meeting him
    直接本人に会わなければだめだ　　　　　　　　You should go and talk to her directly
    ちょっとの時間なら木曜日の放課後に会えるよ　I can see you for a few minutes after class on Thursday

**沿う** そう `to run along; to run beside; to stick to (a line); to follow (a plan, etc.); to act in accordance with`

    川に沿って行く　　　　　　　　　　go along a river
    通りに沿った店　　　　　　　　　　shops along a street
    川に沿って走る　　　　　　　　　　run parallel to a river
    海岸に沿って走る　　　　　　　　　skirt along the coast
    尾根に沿って歩く　　　　　　　　　walk along the ridge of a mountain
    陸地に沿って航行する　　　　　　　follow the coastline
    点線に沿って紙を折る　　　　　　　fold a paper along the dotted line
    川に沿って温泉宿が並んでいる　　　Hot-spring hotels line the river
    この赤い線に沿って進んでください　Please follow the red line

**浮く** うく `to float; to become merry; to become loose; to become unsteady; to feel out of it; to be cut off`

    石は浮かない　　　　　　　　　　　　　　　　Stones don't float
    歯が浮いて噛むと痛む　　　　　　　　　　　　My teeth feel loose and hurt when I chew
    油は水に浮く性質がある　　　　　　　　　　　Oil has the property to float on water
    白い雲が夏の青空に浮いている　　　　　　　　White clouds are floating in the blue summer sky
    彼女は職場で浮いた存在になっている　　　　　She gets left out at work
    石油の特質の一つは水に浮くということである　One of the qualities of oil is that it floats on water

**咲く** さく `to bloom; to flower; to blossom; to open`

    可憐に咲く　　　　　　　　　bloom sweetly
    精一杯咲く　　　　　　　　　bloom vigorously
    桜の咲くころ　　　　　　　　when cherry blossoms bloom
    桜が咲いてるよ　　　　　　　The cherry blossoms are blooming
    この花は咲いたばかりだ　　　This flower has just opened
    春が来ると木の花が咲きます　When spring comes, the trees bloom

**注ぐ** つぐ `to pour (into a vessel); to fill (a cup, bowl, etc.) with; to dish out (food or drink)`

    茶をつぐ　　　　　　　　　　　　　pour tea
    酒をつぐ　　　　　　　　　　　　　fill a cup with sake
    盃に半分酒をつぐ　　　　　　　　　fill a halfway with sake
    もう１杯つぎましょうか　　　　　　Do you want a refill?
    もう少しついでください　　　　　　Please pour me some more
    彼にビールをついであげてください　Could you pour him some beer?

**注ぐ** そそぐ `to pour (into); to sprinkle; to water to pour onto; to spray; to flow into; to drain into; to devote to`

    涙を注ぐ　　　　　　　　　　　　　　　　　　　shed tears
    田畑に水を注ぐ　　　　　　　　　　　　　　　　spray a field with water
    葉の上から水を注ぐ　　　　　　　　　　　　　　water the leaves (of a plant)
    ボトルの中身を注ぎ出す　　　　　　　　　　　　pour the contents out of a bottle
    彼女は洗面器に水を注いだ　　　　　　　　　　　She poured water into the basin
    彼女はカップに熱湯を注いだ　　　　　　　　　　She poured boiling water into the cup
    縁側に暖かい光が注いでいた　　　　　　　　　　The warm sun bathed the veranda
    彼女はカップにミルクを注いだ　　　　　　　　　She poured milk into the cup
    信濃川は新潟で海に注いでいる　　　　　　　　　The Shinano River flows into the sea at Niigata
    彼は友人を助けることに全力を注いだ　　　　　　He put all his energy into helping his friend
    いくつかの小さな流れがその湖に注いでいる　　　The lake is fed by several small streams
    彼女はテーブルの上のカップにコーヒーを注いだ　She poured coffee into a cup on the table

**抜く** ぬく `to pull out; to draw out; to extract; to unplug; to omit; to leave out; to carry through; to let out`

    昼食を抜く　　　　　　　　　　skip lunch
    大根を抜く　　　　　　　　　　pull up a white radish
    白髪を抜く　　　　　　　　　　pull out a white hair
    虫歯を抜く　　　　　　　　　　pull out a decayed tooth
    電源を抜く　　　　　　　　　　pull out the plug
    車のキーを抜く　　　　　　　　take the key out of the ignition
    風呂の湯を抜く　　　　　　　　let the water out of the bathtub
    ワインの栓を抜く　　　　　　　uncork a bottle of wine
    バターを抜いたケーキ　　　　　a cake made without butter
    豚肉をゆでて脂を抜く　　　　　boil pork to remove the fat
    指に刺さったとげを抜く　　　　pull a thorn out of one's finger
    引き出しを抜いてタンスを運ぶ　pull out the drawers and carry the chest of drawers
    必要な書類をファイルから抜く　extract the necessary document from the files
    重要な部分だけ抜いてメモする　extract the important points and note them down
    いる物だけ抜いてあとは捨てた　I picked out the things I needed and threw away the rest

**拭く** ふく `to wipe; to dry`

    額を拭く　　　　　　　　　　　　　　　wipe one's forehead
    汗を拭く　　　　　　　　　　　　　　　wipe the sweat off
    涙を拭く　　　　　　　　　　　　　　　wipe one's tears
    彼女はタオルで顔を拭いた　　　　　　　She wiped her face with a towel
    テーブルを拭いてもらえる　　　　　　　Can you wipe the table for me?
    私は窓の下枠をペーパータオルで拭いた　I wiped the bottom frame of the window with a paper towel

**押す** おす `to push; to press; to apply pressure from above; to press down; to stamp; to affix; to press`

    押さないでください　　　　　　　　　　　Don't push, please
    押すとドアが開いた　　　　　　　　　　　I pushed and the door opened
    彼は肩でドアを押して入ってきた　　　　　He pushed the door (open) with his shoulder and came in
    私は後ろから押されて前につんのめった　　I was pushed from behind and tripped forward
    何度もベルを押したがだれも出てこない　　I pressed the bell again and again but no one came out
    この扉は押しても引いてもびくともしない　This door won't budge when I push or pull
    ぬかるみにはまった車を押すのを手伝った　I helped to push the car out of the mud

**射す** さす `to shine`

    雲間が開け光が射した　　　　　　The clouds opened up and the light shone
    雲が切れて日がさしてきた　　　　The clouds have parted and the sun is shining
    冬の日は部屋の奥までさす　　　　On a winter's day, the sun shines deep into the room
    木が多すぎると家に光が射さない　If there are too many trees, there will be no light in the house

**撃つ** うつ `to shoot (at); to attack; to defeat; to destroy; to avenge`

    撃たないでください　　　　　　　　Please don't shoot
    敵の不意を討つ　　　　　　　　　　Attack the enemy by surprise
    彼女は左腕を撃たれた　　　　　　　She was shot in the left arm
    彼は誤って鉄砲で撃たれた　　　　　He was accidentally shot with a gun
    今までに銃を撃ったことあるですか　Have you ever fired a gun before?
...
**合う** あう `to come together; to merge; to unite; to meet; to fit; to match; to suit; to agree with; to be correct`  
**添う** そう `to meet (wishes, expectations, etc.); to satisfy; to comply with; to live up to; to accompany; to go with`  
**置く** おく `to put; to place; to leave (behind); to establish; to set up; to appoint; to do something in advance`  
**裂く** さく `to tear; to rip up; to cut up; to cleave; to cut open; to forcibly separate; to spare (time, money, etc.)`  
...
**込む** こむ `to be crowded; to be packed; to be congested; to be thronged (with); to be complex; to be intricate; to go into`  
**止む** やむ `to cease; to stop; to be over`  
**売る** うる `to sell`  
**解く** とく `to untie; to unfasten; to unwrap; to undo; to unbind; to unpack; to unstitch; to solve; to work out; to answer`  
...
**合う** あう `to come together; to merge; to unite; to meet; to fit; to match; to suit; to agree with; to be correct`  
    ...
    彼とは馬が合う　　　　　　　　　　　　　He's a good match
    二人の話は合っている　　　　　　　　　　Their two accounts are consistent
    視線が合ったので会釈した　　　　　　　　Our eyes met and I bowed slightly
    血液型が合えば輸血できる　　　　　　　　If the blood types match, we can transfuse them
    われわれの意見はいつも合わない　　　　　Our opinions always differ
    それでは話が合わないじゃないか　　　　　Now you are saying something different
    彼女は彼と性格が合わなくて離婚した　　　She divorced him because she and he didn't get along
    私は彼女と趣味が合うので友だちになった　I became friends with her because we shared the same interests
    
**裂く** さく `to tear; to rip up; to cut up; to cleave; to cut open; to forcibly separate; to spare (time, money, etc.)`

    袋の口を裂く　　　　　　　　　　　　　　　rip open a bag
    この布はうまく裂けない　　　　　　　　　　This cloth is hard to tear
    彼はその新聞を半分に裂いた　　　　　　　　He tore that newspaper in half
    私は彼女との仲を父に裂かれた　　　　　　　My father made me split up with her
    胸を裂かれるような思いだった　　　　　　　I felt as if my heart was being torn 
    このチーズは裂いて食べるとよい　　　　　　This cheese should be ripped open and eaten
    彼は本をばらばらに裂いてしまった　　　　　He tore the book to pieces
    貴重なお時間の数分を裂いていただけますか　Can you spare a few minutes of your precious time?

------------------------------------------------------------------------

###### C3

**遭う** あう `to meet; to encounter; to see; to have an accident; to have a bad experience`  
**問う** とう `to ask; to inquire; to blame (someone) for; to accuse of; to charge with; to care about; to call into question`  
...
**履く** はく `to put on (lower-body clothing, e.g. pants, skirt, footwear); to wear; to affix`  
**蒔く** まく `to sow; to plant; to seed; to sprinkle`  
**消す** けす `to erase; to delete; to cross out; to turn off (power); to switch off; to extinguish; to put out`  
**出す** だす `to take out; to get out; to reveal; to show; to submit; to turn in; to publish; to send; to produce; to begin`  
**勝つ** かつ `to win; to gain victory`  
**飛ぶ** 飛ぶ `to fly; to soar; to jump; to leap; to spring; to bound; to hop; to scatter; to flee`  
...
**得る** える `to get; to earn; to acquire; to gain; to secure; to attain; to obtain; to win; to comprehend; to be able to`  
**打つ** うつ `to hit; to strike; to knock; to beat; to punch; to slap; to tap; to bang; to clap; to pound; to strike; to beat`  

------------------------------------------------------------------------

**遭う・会う** あう `to meet; to encounter; to see; to have an accident; to have a bad experience`  

    ボストンで会った　　　　　　　　　　　　We met in Boston
    また会いましょう　　　　　　　　　　　　I'll see you soon
    君とは初めて会った気がしない　　　　　　I don't feel like I've never seen you before
    ３時にお得意さんと会う約束がある　　　　I have an appointment with a client at 3:00
    彼は彼女と土曜の午後に会う約束をした　　He'd arranged to meet her Saturday afternoon
...
**保つ** たもつ `to keep; to preserve; to hold; to retain; to maintain; to sustain`
    ...
    安定を保つ　　　　　　　　　　　　　maintain stability
    調和を保つ　　　　　　　　　　　　　preserve harmony
    秩序を保つ　　　　　　　　　　　　　keep order
    記録を保つ　　　　　　　　　　　　　keep a record
    体面を保つ　　　　　　　　　　　　　maintain appearances
    潤いを保つ　　　　　　　　　　　　　retain moisture
    温度を一定に保つ　　　　　　　　　　keep the temperature at a fixed level
    彼は冷静さを保っていられなかった　　He wasn't able to maintain his composure

------------------------------------------------------------------------

###### SS

**差す** さす `to shine; to be visible; to be tinged with; to rise (of water levels); to flow in; to be felt`  
...
**注す** さす `to pour; to add; to serve (drinks); to put on (lipstick, etc.); to apply; to colour; to light (a fire)`  
**射す** さす `to shine`  
**挿す** さす `to insert; to put in; to plant (a cutting); to strike; to arrange (flowers); to shut; to close`  

**着る** きる `to wear (in modern Japanese, from the shoulders down); to put on; to bear`  
**切る** きる `to cut; to cut through; to perform (surgery); to sever; to turn off; to terminate; to disconnect`  
...
**付く** つく `to be attached; to be connected with; to adhere; to stick; to cling; to scar; to stain; to bear`  
**就く** つく `to take (seat, position, course, office, etc.); to assume; to be hired; to be employed`  
**点く** つく `to be lit (e.g. electricity comes on); to be lighted; to catch fire​`  
**突く** つく `to prick; to stab; to poke; to prod; to push; to thrust; to nudge; to hit; to strike; to press against`  
**吐く** つく `to breathe out; to breathe; o tell (a lie); to use (foul language); to vomit; to throw up; to spit up`  
**搗く** つく `to hull (rice, barley, etc.); to pound (rice); to polish (rice); to stamp (ore)`  
**漬く** つく `to be immersed`  
...
<!--     うえの棚には辞書ばかり置いてあります -->
<!--     There are only dictionaries on the top shelf -->
...
A.1 **あいだ** `throughout` **⚭ Vru/Vteiru/Na-Adj+na/N+no + aida**  
...
A.2 **あいだに** `during; when` **⚭ Vru/Vteiru/Na-Adj+na/N+no + aida ni**
...
A.3 **あげく** `after much; ended up; in the end; after all` **⚭ Vta/N+no + ageku**
...
<!-- <h> -->
    ...
    ・After getting lost for a long time, he finally had to go back to the station and ask for directions at the police station.
    ・After worrying his parents a lot about money and friends, Taro finally left home.
    ・After a long discussion on this issue, the conclusion was postponed.
    ・After many days of discussion, we decided not to send a representative this time.

A.4 **あげる** `give` **⚭ N + wo + ageru**
...
<!-- <h> -->
    ...
    ・My sister gave Aiko a cake for her birthday.
    ・A: Wow, you bought a lot of souvenirs. B: Yes, I'm giving them to the people at work.
    ・I bought this doll to give to the director's wife.
    ・Let's give her something to celebrate her award.
    ・Don't forget to water the flowers during my trip.
    ・A: Do you give New Year's gifts to your children at New Year's? B: No, I wouldn't do it to my own children.
    
A.5 **あっての** `comprised of; dispensable to` **⚭ N + atteno**

    愛あっての結婚生活だ。愛がなければ、いっしょに暮らす意味がない。
    わたしたちはお客さまあっての仕事ですから、お客さまを何より大切にしています。
    交渉は相手あってのことですから、自分の都合だけ主張してもうまくいかない。

A.6 **後で** `after` **⚭ Vta/N+no + atode**
...
A.7 **あまり** `so much.. that` **⚭ Vplain/Na-Adj+na/N+no + amari**
...
A.8 **あまりの** `so.. that` **⚭ amari + no + N + ni**
...
I.1 **いかんで** `in accordance with・is contingent on` **⚭ N+(no) + ikande**  
...
**遠慮なく** `without hesitation・without reservation・freely・frankly` **⚭ Adverb**  
`・Expression・Adverb`
...
<!-- <h> -->
    ...
    ・Please feel free to give us your opinions
    ・If you have any questions, please feel free to ask

**かまいません** `no problem・it doesn't matter` **⚭ Expression**  
`・Expression`
    ...
    ・「今、ちょっとよろしいでしょうか？」「ええ、かまいませんよ。どうしたんですか」
...
<!-- <h> -->
    ...
    ・You are welcome to come whenever it is convenient for you
    ・May I have a moment? "Yes, of course. What's going on?
...
**抜く** `do something to the end・extremely・completely・thoroughly・very` **⚭ V~~masu~~ + nuku**  
`① 苦しくても最後まで頑張って~する・完了する　V: やる・頑張る・生きる・走る・守る・耐える・戦う`    
`② 徹底的・そごく・非常に~する　　　　　　　　V: 考える・悩む・困る・苦しむ・知る・選ぶ`
...
# blgmr <- blgmr[sample(1:nrow(blgmr)),]
...
reactable(
  ...
  columns = list(
      Item = colDef(
       cell = function(value, index) {
        Meaning <- blgnn$Meaning[index]
        Meaning <- if (!is.na(Meaning)) Meaning else "Unknown"
        Definition <- blgnn$Definition[index]
        Definition <- if (!is.na(Definition)) Definition else "Unknown"
        Pattern <- blgnn$Pattern[index]
        Pattern <- if (!is.na(Pattern)) Pattern else "Unknown"
        tagList(
          div(style = list(fontSize = 17, fontWeight = 500), value),
          # div(style = list(fontSize = 10, fontWeight = 200), Meaning),
          div(style = list(fontSize = 10, fontWeight = 200), Definition),
          div(style = list(fontSize = 10, fontWeight = 200), Pattern)
        )
      },
      align = "center", minWidth = 300
       ...
       Sentence = colDef(
       cell = function(value, index) {
        Translation <- blgnn$Translation[index]
        Translation <- if (!is.na(Translation)) Translation else "Unknown"
        tagList(
          div(style = list(fontSize = 17, fontWeight = 500), value),
          div(style = list(fontSize = 11, fontWeight = 200), Translation)
      ...
      align = "center", minWidth = 500
      ),
      Meaning = colDef(show = FALSE),
      Definition = colDef(show = FALSE),
...
reactable(
  ...
  columns = list(
       Sentence = colDef(
       cell = function(value, index) {
        Translation <- blgmr$Translation[index]
        Translation <- if (!is.na(Translation)) Translation else "Unknown"
        tagList(
          div(style = list(fontSize = 19, fontWeight = 500), value),
          div(style = list(fontSize = 11, fontWeight = 200), Translation)
    ...
    outlined = TRUE, defaultPageSize = 7, compact = FALSE, highlight = TRUE, fullWidth = TRUE,
  showPageInfo = FALSE, bordered = TRUE, searchable = TRUE, sortable = FALSE, paginationType = "numbers",
  style = list(fontFamily = "Heiti SC, Hiragino Maru Gothic ProN, Anonymous Pro , monospace, Helvetica Neue", fontSize = "11px"),
    theme = reactableTheme(
    # Vertically center cells
...
ilgmr <- ilgmr[sample(1:nrow(ilgmr)),]
...
reactable(
  ...
  columns = list(
       Sentence = colDef(
       cell = function(value, index) {
        Translation <- ilgmr$Translation[index]
        Translation <- if (!is.na(Translation)) Translation else "Unknown"
        tagList(
          div(style = list(fontSize = 19, fontWeight = 500), value),
          div(style = list(fontSize = 11, fontWeight = 200), Translation)
...
algmr <- algmr[sample(1:nrow(algmr)),]
...
reactable(
  ...
  columns = list(
       Sentence = colDef(
       cell = function(value, index) {
        Translation <- algmr$Translation[index]
        Translation <- if (!is.na(Translation)) Translation else "Unknown"
        tagList(
          div(style = list(fontSize = 19, fontWeight = 500), value),
          div(style = list(fontSize = 11, fontWeight = 200), Translation)
...
reactable(
  ...
  style = list(fontFamily = "Heiti SC, Hiragino Maru Gothic ProN, Anonymous Pro , Helvetica Neue", fontSize = "11px")
)
```
...
cmpdv <- cmpdv[sample(1:nrow(cmpdv)),]
...
reactable(
  ...
  columns = list(
      Word = colDef(
      cell = function(value, index) {
        Reading <- cmpdv$Reading[index]
        Reading <- if (!is.na(Reading)) Reading else "Unknown"
        Meaning <- cmpdv$Meaning[index]
        Meaning <- if (!is.na(Meaning)) Meaning else "Unknown"
        tagList(
          div(style = list(fontSize = 19, fontWeight = 600), value),
          div(style = list(fontSize = 11, fontWeight = 200), Reading),
...
reactable(
  ...
  columns = list(
    Script = colDef(
      # Show species under character names
      cell = function(value, index) {
        Translation <- ctad$Translation[index]
        Translation <- if (!is.na(Translation)) Translation else "Unknown"
        tagList(
          div(style = list(fontSize = 19, fontWeight = 500), value),
          div(style = list(fontSize = 11), Translation)
...
wrdmm <- wrdmm[sample(1:nrow(na.omit(wrdmm))),]

reactable(
  na.omit(wrdmm),
  defaultPageSize = 10, 
  columns = list(
    w1 = colDef(minWidth = 75, style = list(fontSize = 13, fontWeight = 500), align = "center"), 
    w2 = colDef(minWidth = 75, style = list(fontSize = 13, fontWeight = 500), align = "center"), 
    w3 = colDef(minWidth = 75, style = list(fontSize = 13, fontWeight = 500), align = "center"), 
    w4 = colDef(minWidth = 75, style = list(fontSize = 13, fontWeight = 500), align = "center"), 
    w5 = colDef(minWidth = 75, style = list(fontSize = 13, fontWeight = 500), align = "center"), 
    w6 = colDef(minWidth = 75, style = list(fontSize = 13, fontWeight = 500), align = "center"), 
    w7 = colDef(minWidth = 75, style = list(fontSize = 13, fontWeight = 500), align = "center"), 
    w8 = colDef(minWidth = 75, style = list(fontSize = 13, fontWeight = 500), align = "center"), 
    w9 = colDef(minWidth = 75, style = list(fontSize = 13, fontWeight = 500), align = "center"), 
    w10 = colDef(minWidth = 75, style = list(fontSize = 13, fontWeight = 500), align = "center")
  ),
  theme = reactableTheme(
    # Vertically center cells
...
reactable(na.omit(kc2), showPageSizeOptions = FALSE, 
          ...
          style = list(fontFamily = "Heiti SC, Hiragino Maru Gothic ProN, Anonymous Pro , Helvetica Neue", fontSize = "11px", fontWeight = 350),
          columns = list(
                Kanji = colDef(
                cell = function(value, index) {
                  tagList(
...
解散`breaking up (e.g. meeting); dispersing (e.g. crowd)`   
...
会計`accounting; finance; account; reckoning; bill (at a restaurant); check`   
...
消費税`consumption tax (incl. sales tax, VAT, excise duty, etc.)`   
...
使用料`use fee; rent`  
手数料`(handling) fee; charge (e.g. for a cancellation); commission; brokerage`   
...
燃料`fuel`  
資料`materials; data; document`  
原料`raw materials`  
衣料`clothing`  
試料`sample; specimen; test piece`  
食料品`foodstuff; groceries`  
調味料`seasoning; flavoring; flavouring; condiment`  
香辛料`spice; spices; condiment`  

報告`report; information`   
申告`report; return (e.g. tax); statement; declaration`   
報道`report (of news); reporting; news; informatio`   
記事`article; news story; report; account`   
...
煮炊き`cooking`   
割烹`cooking (esp. Japanese style); cuisine`   
日本料理`Japanese cooking`   
中華料理`Chinese cooking; Chinese food`   
炊さん`cooking (rice, grains)`   
手料理`home cooking`   
...
`cooking; cookery; cuisine; meal; food; dish; item on a menu` 料理  
`understanding; comprehension; appreciation; sympathy` 理解  
`ideal; ideals` 理想  
`representation; agency; proxy; deputy; agent; attorney; substitute` 代理  
`principle; theory; fundamental truth` 原理  
`theory` 理論  
`unreasonable; unnatural; unjustifiable; impossible` 無理  
`geography` 地理  
`reason; reasoning power; (one's) sense` 理性  
`truth` 真理  
`cooking; food preparation` 調理  
`duty; sense of duty; honor; honour; decency; courtesy; debt of gratitude` 義理  
`repair; mending; fixing; servicing` 修理  
`processing; dealing with; treatment; disposition; disposal` 処理  
`control; management (e.g. of a business)` 管理  
`physics; law of nature` 物理  
`sorting; arrangement; organization; putting in order; adjustment; regulation` 整理  
`state of mind; mentality; psychology` 心理  
`science (inc. mathematics, medicine, etc.); natural science` 理科  
`physiology; menstruation; one's period; menses` 生理  
`theory; reason; logic; (unreasonable) argument; strained logic` 理屈  
`logic` 論理  
`reasoning; inference; deduction` 推理  

`to leak; to run out; vi` 漏る  
`to leak out; to escape; to come through; to shine through; to filter out;  to be disclosed; to be left out; vi` 漏れる  
`to let leak; to let out (e.g. light); to leak; to disclose; to omit; to leave out; vt` 漏らす  
`to serve (on a plate); to dish out; to pile up; to heap up; to administer; to put into (info); to mark out; vt (moru)` 盛る  
`to prosper; to flourish; vi (sakaru)` 盛る  
`prosperous; flourishing; thriving; successful; popular; widespread` 盛ん  

`cost; expense` 費用  
`quotation; citation; reference` 引用  
`practical use; application` 活用  
`use; utilization; utilisation; application` 利用  
`preparation; arrangements; provision; getting ready; laying out (e.g. a meal)` 用意  
`abuse; misuse; misappropriation; using to excess` 乱用  
`tasks; things to do; errand; business (to take care of); affairs; engagement` 用事  
`confidence; trust; faith; reputation` 信用  
`use; application; employment; utilization; utilisation` 使用  
`applying (e.g. a technology); adoption` 適用  
`on business; for business; business purpose` 商用  
`care; precaution; guarding; caution` 用心  
`skillful; skilful; adroit; dexterous; dextrous; deft; nimble; handy; clever` 器用  
`(practical) application; putting to practical use` 応用  
`term; terminology` 用語  
`daily necessities` 日用品  
`practical use; utility​` 実用  
`use; service; purpose` 用途  
`popular use; circulation` 通用  
`use; adoption; acceptance` 採用  
`blank form` 用紙  
`employment (long term); hire` 雇用  
`substitution` 代用  
`(one's) exclusive use; private use; personal use; dedicated use` 専用  
`useless; futile; unnecessary; needless` 無用  
`articles; supplies; parts` 用品  
`official business; government business; public business; company duties; public use; government use` 公用  
`business; thing to be done; something that should be performed; information that should be conveyed` 用件  
`employee; servant` 使用人  
`personal use; private use` 私用  
`directions; rules of use` 用法  
`multi-use; combined use; combination; serving two purposes` 兼用  
`side effect; adverse reaction` 副作用  
`action; operation; effect; function` 作用  
`full employment` 完全雇用  
`interaction` 相互作用  
`useful; helpful` 有用  
`taking medicine; dosing` 服用  
`wearing (e.g. uniform, seat belt); having on` 着用  
`common use; communal use; sharing` 共用  
`for use as food; edible` 食用  
`unprepared` 不用意  
`site` 用地  
`user; end-user; consumer` 利用者  
`habitual use; daily use` 常用  
`tool; implement; instrument; equipment` 用具  
`favorite; favourite; habitually used` 愛用  
`user; consumer` 使用者  
`using together (jointly); used at the same time` 併用  
`appointment (to a position, job, etc.); being used for a role; promotion` 起用  
`diversion; putting something to another use` 転用  
`(passenger) car; automobile` 乗用車  
`clumsy; awkward; unskillfulness; lack of ability` 不器用  
`urgent business` 急用  
`practical; useful; utilitarian; pragmatic` 実用的  
`careful; cautious; wary; watchful; vigilant; prudent` 用心深い  
`example; illustration` 用例  
`writing implement; writing tool` 筆記用具  
`abuse; misuse; perversion` 悪用  
`for use by children; for children` 子供用  
`chores; odd jobs; miscellaneous affairs` 雑用  
`misuse; incorrect use; abuse; malapropism` 誤用  
`external use` 外用  
`drinking` 飲用  
`official language` 公用語  
`exclusive line (communications); dedicated line` 専用線  
`making use of; application; practical use; effective management (e.g. of funds)` 運用  
`customary use; common usage; general usage` 慣用  

伝統食`traditional food; ancestral food`  
現代風`modern style; modernness`  
造り手`maker; builder; creator; tenant farmer`
拘り`obsession; fixation; hangup; determination; fastidiousness; pickiness about (trait, style, etc.)`  
想い`thought; imagination; mind; heart; desire; wish; hope; love; affection; feelings`  
詰まる`to be packed (with); to be full (space, schedule, etc.); filled with; to be blocked`  
品々`various articles`  
日本各地`all over Japan`  
厳選`careful selection; careful screening; hand-picking`  
届ける`to deliver; to forward; to send; to report; to notify;`  
ご褒美`reward; prize`  
食卓`dining table`  
彩る`to colour; to color; to paint; to apply make-up; to decorate; to garnish; to adorn; to add flair`  
地方`district; region; area; locality`  
究極`ultimate; extreme; final; eventual`  
親子丼`oyakodon; chicken and egg on rice`  
お強`glutinous rice steamed with red beans or some other ingredient`  
逸品`article of rare beauty; gem`  
手毬`traditional Japanese handball game`  
見た目`appearance`  
零れる`to spill; to fall out of; to overflow; to peek through; to become visible; to escape (smile, etc)`  
あでやか`glamorous; glamourous; charming; beguiling; bewitching; beautiful; fascinatingly elegant`   
歓声`cheer; shout of joy`  
拘る`to fuss over; to be particular about; to be obsessive about; to be fixated on; vi`  
関わる`to be affected; to be influenced; to be concerned with; to have to do with; vi`  
贅沢`luxury; extravagance; to live in luxury`  

手先が器用`being skilled with one's hands`
なんかこういう`something like this`
...
#### A
    ...
    わたしは夏の間ずっと北海道にいました
    両親が旅行をしている間ぼくが毎日食事を作りました
    兄がゲームをしている間弟はそばで見ています
    夏休みにとなりのうちがにぎやかな間はわたしも楽しい気分になる
    夏休みの間に引っ越ししたいです
    わたしがいない間にだれか来たのでしょうか。門が少し開いています
    日本にいる間に１度富士山に登りたい
    暇な間に本だなをかたづけてしまおう
...
#### A
    ...
    あいかわらず　　　・相変わらず　　　・父はあいかわらず、毎日２箱タバコを吸っている　　　　　　・As before, as usual
    あいきょう　　　　・愛嬌　　　　　　・このパンダの縫いぐるみはあいきょうがある　　　　　　　　・Charming, Cute
    あいさつ　　　　　・挨拶　　　　　　・彼は道で会っても挨拶さえしてくれない　　　　　　　　　　・A greeting
    あいじょう　　　　・愛情　　　　　　・先生の学生たちへの愛情は深い　　　　　　　　　　　　　　・Love, Affection
    あいず　　　　　　・合図　　　　　　・彼女に座るように手で合図した　　　　　　　　　　　　　　・A signal, A sign
    あいする　　　　　・愛する　　　　　・愛する人とはいつもー緒にいたいと思うものだ　　　　　　　・Love, Care for

------------------------------------------------------------------------

#### I

    いにそう　　　　　・意に沿う　　　　・注文した客の意にそうように家を設計する　　　　　　　　　・Go along with a person's desire
    いにはんする　　　・意に反する　　　・むかしは自分の意に反する結婚をする人も多かった　　　　　・Act against a person's will
    いのまま　　　　　・意のまま　　　　・やっと意のままに運転できるようになった　　　　　　　　　・At will
    いあわせる　　　　・居合わせる　　　・その場に居合わせた人は多くはなかった　　　　　　　　　　・Happen to be present

------------------------------------------------------------------------

#### U

    うえる　　　　　　・植える　　　　　・山に杉の木を植える・か花を植えたらすぐ水をやりなさい　　・To plant
    うかがう　　　　　・伺う　　　　　　・さっきのお話をもうすこしくわしくうかがいたいのですが　　・Hear, Ask, Visit
    うかぶ　　　　　　・浮かぶ　　　　　・池に小さな舟が浮かんでいる・木の葉が水に浮かぶ　　　　　・Float
    うかぶ　　　　　　・浮かぶ　　　　　・大きな風船が空に浮かんでいる　　　　　　　　　　　　　　・Float
    うかぶ　　　　　　・浮かぶ　　　　　・悲しい知らせを聞いた人々の目には涙が浮かんだ　　　　　　・Come to, Occur to
    うかぶ　　　　　　・浮かぶ　　　　　・幼いころの思い出が心に浮かぶ　　　　　　　　　　　　　　・Come to, Occur to
    うかべる　　　　　・浮かべる　　　　・ふろの水におもちゃを浮かべて遊ぶ・舟を浮かべる　　　　　・To float
    うかべる　　　　　・浮かべる　　　　・ほほえみを浮かべてあいさつする・思い浮かべる　　　　　　・Express, Wear

------------------------------------------------------------------------

#### SE

    せいかい　　　　　・正解　　　　　　・入試問題の正解をきく・正解を出す　　　　　　　　　　　　・A correct answer 
    せいかい　　　　　・正解　　　　　　・あ、雨が降りだした。傘を持ってきたのは正解だった　　　　・Be right to
    せっち　　　　　　・設置　　　　　　・各研究室に冷房装置を設置する・消火器の設置を義務づける　・Install, Furnish
    せっち　　　　　　・設置　　　　　　・区内に図書館を設置するか・事故の調査委員会を設置する　　・Establish, Found, Set up
    
------------------------------------------------------------------------

#### YO

    よあけ　　　　　　・夜明け　　　　　・夜明けの空に小鳥の声が聞こえる・夜明けが近い　　　　　　・Dawn, Daybreak
    よい　　　　　　　・酔い　　　　　　・酔いがまわって頭がふらふらする・酔いをさます・乗り物酔い・Drunkenness, Motion sickness
...
1.1 **あまり ~ない**`not very・not much` **⚭ amari + Vnai/N+janai/I-Adj~~i~~+kunai/Na-Adj+janai**

    ・鶏肉はあまり好きじゃないです  
    ・I don't really like chicken

1.2 **後で**`after・later` **⚭ Vta/N+no + atode**

    ・昼ごはんの後で友達とお茶を飲みました  
    ・After lunch, I had a cup of tea with a friend

1.3 **ば**`if..then` **⚭ (Vcmd-stem + ba) → Vba**

    ・このボタンを押せばドアが開きます  
    ・If you press this button, the door wil open

1.4 **場合(は)**`if・in the case・in the event of` **⚭ Vplain/N+no/Na-Adj+na + baai wa**

    ・もし無理な場合は注文をキャンセルします 
    ・If this is not possible, We will be cancel the order

1.5 **場合(に)**`if・in the case・in the event of` **⚭ Vplain/N+no/Na-Adj+na + baai ni**

    ・ロブスターは特別な場合にしか食べない
    ・I only eat lobster on special occasions

1.6 **だけで**`just by・just by doing` **⚭ Vplain/N + dakede**
...
1.1 **別に~ない**`ifn(Expressing that nothing is special)` **⚭ betsu ni + Vnai/N+dewanai/I-Adj~~i~~=kunai/Na-Adj+dewanai**
...
16.1 **ようなら**`if (if something/someone in this situation, then)` **⚭ Vplain/I-Adj/N+no/Na-Adj+na + you nara**
...
1.1 **うちに**`before・while (do something before a state or a situation changes)` **⚭ Vplain/I-Adj/Na-Adj/N+no + uchini**

    ・溶けないうちに早く食べてください 
    ・Eat quickly before it melts

1.2 **うちに**`while (express a process or action is happening a change also happens)` **⚭ Vplain/I-Adj/Na-Adj/N+no + uchini**

    ・音楽を聞いているうちに眠ってしまった 
    ・I fell asleep while listening to the music

3.1 **てからでないと**`until・unless (until or unless something happens/done, it cannot happens/be done)` **⚭ Vte + tekaradenaito**
...
3.2 **てからでなければ**`until・unless (unless something happens/done, it cannot happen/be done)` **⚭ Vte + tekaradenakereba**
...
1.2 **には・のに(は) ＋ ..が必要だ・が便利だ・なければならない・がかかる**
...
2.1 **まもなく・もうすぐ・やがて・来年 + ..Vru** `Words that express time and verb forms must agree`

    ・来年この子は５歳になる 

2.2 **今・現在 + ..Vteiru** `Words that express time and verb forms must agree`

    ・現在雪が降っている 

2.3 **去年・先週 + ..Vta** `Words that express time and verb forms must agree`
...
1.1 **ないと**`have to (should or have to do)` **⚭ V~~nai~~ + nakya/nakucha/naito**

    ・あ、もう８時だ。そろそろ出かけないと
    ・Oh, it's 8 o'clock already. I have to go out soon

1.2 **っけ**`what was it? (confirm an info that was forgotten)` **⚭ Vru(nda/ndatta)/Vta/I-Adj+katta/(Na-Adj/N)+da/datta+ kke**

    ・これどうやって食べるんだったっけ
    ・How should I eat this again?

1.3 **疑問視 + か**`cite or refer to a question` **⚭ Question word ~　Vru + ka + (kiku/shitteiru/wakaru/oshieru/oboeteiru..)**

    ・何を買うかわからない
    ・I don't know what I'll buy

1.4 **かどうか**`whether or not (cite or refer to a question)` **⚭ Vru + ka + (kiku/shitteiru/wakaru/oshieru/oboeteiru..)**

    ・出かける前に鍵をかけるかどうか覚えていません
    ・I can't remember if I lock the door before I leave

1.5 **ている**`continuous action (progressive)` **⚭ Vte + iru**

    ・あそこで新聞を読んでいる人知っている？
    ・Do you know who's reading the newspaper over there?

1.6 **ている**`continuous state (resultative)` **⚭ Vte + iru**

    ・いい天気です。空が晴れています
    ・It's a beautiful day. The sky is clear.

1.7 **ている**`habitual・routine state (repeatitive)` **⚭ Vte + iru**

    ・あの店では新しい野菜を売っています
    ・That vegetable store sells fresh vegetables

1.8 **てしまう・ちゃう**`whether or not (cite or refer to a question)` **⚭ Vte + shimau / V~~te/de~~ + chau/jau**

    ・小遣いはもう使ってしまったの?
    ・Have you spent your allowance already?

1.9 **ておく・とく**`to do something in advance` **⚭ Vte + oku / V~~te~~ + toku**

    ・このビールを冷蔵庫に入れといて
    ・Put this beer in the fridge (in advance)

1.10 **ようにいう**`says・tells (tells, cite or refer to an order, request, or piece of advice)` **⚭ Vru/Vnai + youni iu)**

    ・先生はみんなにもっと勉強するよう言った
    ・The teacher told everyone to study harder

1.11 **ように言われる**`told by someone (tells, cite or refer to an order, request by someone)` **⚭ Vru/Vnai + youni iwareru)**
...
1.1 **あるいは**`or・possibly` **⚭ aruiwa + N/Phrase**

    ・あるいは、あなたの言うとおりかもしれません 
    ・Or maybe you're right

2.1 **もとに**`based on・derived from` **⚭ N + wo + motoni/motonishite**
...
##### QR
    ...
    あるいは　　　　　　　　　　・あるいはそれは本当かもしれない　　　　　　　　　　　　　　　　・or, possibly
    ばかり　　　　　　　　　　　・十マイルばかり歩いたあとで、彼らは立ち止まった　　　　　　　　・about, approximately
    ばかりだ　　　　　　　　　　・私の不満は募るばかりだった　　　　　　　　　　　　　　　　　　・continue to
    ばかりに　　　　　　　　　　・ちょっとした油断をしたばかりに、大怪我をしてしまった　　　　　・because, on account of
    ぶりに　　　　　　　　　　　・彼は三日ぶりに意識を回復した　　　　　　　　　　　　　　　　　・for the first time in
    ちっとも～ない　　　　　　　・彼女は、彼氏と別れたことをちっとも気にかけていない様子だ　　　・not at all, not in the least
    だけあって　　　　　　　　　・彼はスポーツの選手だけあって、体格がいい　　　　　　　　　　　・being the case, precisely because
    だけでなく　　　　　　　　　・彼女は日本語が上手なだけでなく、英語もペラペラだ　　　　　　　・not only..but also
    だけましだ　　　　　　　　　・今日は暑いが、湿度が高くないだけましだ　　　　　　　　　　　　・one should feel grateful for
    だけに　　　　　　　　　　　・この品物は品質がよいだけに値段も高い　　　　　　　　　　　　　・being the case, precisely because
    だけのことはある　　　　　　・君は慎み深い人だ。彼が気に入っているだけのことはある　　　　　・no wonder, it’s not surprising that
    だけは　　　　　　　　　　　・覚えるだけは覚えたのだから、後は試験の日を待つだけだ　　　　　・to do all that one can
    だって　　　　　　　　　　　・何を怒ってるの？　だって、約束を破ったじゃん　　　　　　　　　・because, but
    だって　　　　　　　　　　　・誰だってそんなことは知っているさ　　　　　　　　　　　　　　　・even, too
...
<!-- 中を (naka o)	in, on -->
<!-- なお (nao)	furthermore, still, yet [used to add more information to the sentence] -->
...
<!-- おまけに (omake ni)	to make matters worse -->
...
1.1 **といえば**`speaking of (speaking one's mind)` **⚭ N + toieba**

    ・日本料理といえばはやりお寿司が有名です
    ・Speaking of Japanese cuisine, sushi is famous

1.2 **にもかかわらず**`inspite of (express surprise feeling due to unexoected result)` **⚭ Vplain/I-Adj/Na-Adj/N + nimokakawarazu**

    ・大学生にもかかわらず基本的な漢字が書けない者もいる
    ・Despite of being college studentes there are some who cannot write basic kanji

1.3 **をめぐって**`concerning・inregard to (express a thing that is controvesial)` **⚭ N + wo + megutte**

    ・環境の問題をめぐって多くの意見が出された
    ・Many opinions were expressed concerning the issue of the environment
    
1.4 **にしては**`for・considering it's (express a differentiation between an image and a reality)` **⚭ Vplain/N + ni + shitewa**

    ・初めてにしては上手上手ですね
    ・You're pretty good for your first time

1.5 **げ**`looks like・seems like・appears to (express superficial appearance)` **⚭ Na-Adj/I-Adj~~i~~ + ge**

    ・学生たちは楽しげにおしゃべりしていた
    ・It seems the students were chatting happily

1.6 **からすると**`judging from・considering・by the look of (wants to judge something)` **⚭ N + kara + suruto/sureba/itte**

    ・彼の症状からすると心臓の病気かもしれません
    ・Judging from on his symptoms, he may have a heart condition

1.7 **もしないで**`without even doing (express not ~at all, express discontent)` **⚭ V~~masu~~ + moshinaide**

    ・食べもしないでまずいかどうかなんてわからないでしょ
    ・You can't tell if it's bad or not without eating it

1.8 **くせに**`although・despite the fact that・but (express discontent)` **⚭ Vplain/I-Adj/Na-Adj/N+no + kuseni**

    ・元気なくせに病気のふりをしている
    ・You're fine, but you're pretending to be sick
...
1.1 **際**`when・on the occasion of・at the time of・in the case of` **⚭ Vplain/Vta/N+no + sai (ni)(wa)** *⇔ ~toki ni*

    ・この整理券は商品受け取りの際、必要です
    ・こちらの会議室をご利用になる際は、受付で必要事項を記入ください
    
<!-- <h> -->

    ・This numbered ticket is required in receiving the product
    ・Please fill out the necessary information at the reception desk to use this meeting room

1.2 **に際して**`when・on the occasion of・at the time of` **⚭ Vplain/N + nisaishite** *⇔ ~suru toki ni*

    ・工場関係者は工場を始めるに際して近所の住民に挨拶をして回った
    ・卒業式に際して市長からお祝いの言葉が卒業生に送られた
    
<!-- <h> -->

    ・The factory officials went around to greet the residents of the neighborhood when they started the factory
    ・Please fill out the necessary information at the reception desk to use this meeting room
