...
<!-- reactable( -->
<!--   na.omit(srvb2 %>% slice(65:1300)), -->
<!--   columns = list( -->
<!--     Verbs = colDef( -->
<!--       # Show species under character names -->
<!--       cell = function(value, index) { -->
<!--         Meaning <- srvb2$Meaning[index] -->
<!--         Meaning <- if (!is.na(Meaning)) Meaning else "Unknown" -->
<!--         Reading <- srvb2$Reading[index] -->
<!--         Reading <- if (!is.na(Reading)) Reading else "Unknown" -->
<!--         tagList( -->
<!--           div(style = list(fontSize = 30, fontWeight = 700), value), -->
    ...
    Kanji = colDef(
      cell = function(value, index) {
        ...
        tagList(
          ...
          div(style = list(fontSize = 25, fontWeight = 500), w1, "・", w2, "・", w3),
          div(style = list(fontSize = 25, fontWeight = 500), w4, "・", w5, "・", w6),
          div(style = list(fontSize = 25, fontWeight = 500), w7, "・", w8, "・", w9),
        )
      },
      align = "center"
    ),
  ...
  columns = list(
    Words = colDef(
      cell = function(value, index) {
        ...
        w8 <- if (!is.na(w8)) w8 else "Unknown"
        tagList(
          div(style = list(fontSize = 25, fontWeight = 500), value, "・", w1, "・", w2),
          div(style = list(fontSize = 25, fontWeight = 500), w3, "・", w4, "・", w5),
          div(style = list(fontSize = 25, fontWeight = 500), w6, "・", w7, "・", w8)
        )
      },
      align = "center"
    ),
...
https://nihongoswitch.com/podcast-transcripts/ep66-shadowing-2
...
**Nanji desu ka?**`What time is it`
    ...
    ・２時　　　　　niji               2 o'clock
    ・３時　　　　　sanji              3 o'clock
    ・３時５分　　　sanjigofun         3:05
    ・３時半　　　　sanjihan           3:30
    ・午後３時５分　gogo sanjigofun    3:05 pm
    ・午前３時５分　gozen sanjigofun   3:05 am
...
reactable(
  ...
  columns = list(
    Verbs = colDef(
      # Show species under character names
      cell = function(value, index) {
        Meaning <- (srvb2 %>% slice(595:785))$Meaning[index]
        Meaning <- if (!is.na(Meaning)) Meaning else "Unknown"
        Reading <- (srvb2 %>% slice(595:785))$Reading[index]
        Reading <- if (!is.na(Reading)) Reading else "Unknown"
        tagList(
          div(style = list(fontSize = 30, fontWeight = 700), value),
          div(style = list(fontSize = 13), Reading),
...
reactable(
  ...
  columns = list(
    Verbs = colDef(
      # Show species under character names
      cell = function(value, index) {
        Meaning <- (srvb2 %>% slice(830:853))$Meaning[index]
        Meaning <- if (!is.na(Meaning)) Meaning else "Unknown"
        Reading <- (srvb2 %>% slice(830:853))$Reading[index]
        Reading <- if (!is.na(Reading)) Reading else "Unknown"
        tagList(
          div(style = list(fontSize = 30, fontWeight = 700), value),
          div(style = list(fontSize = 13), Reading),
...
reactable(
  srvb2 %>% slice(1250:1268),
  columns = list(
    Verbs = colDef(
      # Show species under character names
      cell = function(value, index) {
        Meaning <- (srvb2 %>% slice(1250:1268))$Meaning[index]
        Meaning <- if (!is.na(Meaning)) Meaning else "Unknown"
        Reading <- (srvb2 %>% slice(1250:1268))$Reading[index]
        Reading <- if (!is.na(Reading)) Reading else "Unknown"
        tagList(
          div(style = list(fontSize = 30, fontWeight = 700), value),
          div(style = list(fontSize = 13), Reading),
...
reactable(
  srvb2 %>% slice(1269:1297),
  columns = list(
    Verbs = colDef(
      # Show species under character names
      cell = function(value, index) {
        Meaning <- (srvb2 %>% slice(1269:1297))$Meaning[index]
        Meaning <- if (!is.na(Meaning)) Meaning else "Unknown"
        Reading <- (srvb2 %>% slice(1269:1297))$Reading[index]
        Reading <- if (!is.na(Reading)) Reading else "Unknown"
        tagList(
          div(style = list(fontSize = 30, fontWeight = 700), value),
          div(style = list(fontSize = 13), Reading),
...
reactable(
  srvb2 %>% slice(1298:1304),
  columns = list(
    Verbs = colDef(
      # Show species under character names
      cell = function(value, index) {
        Meaning <- (srvb2 %>% slice(1298:1304))$Meaning[index]
        Meaning <- if (!is.na(Meaning)) Meaning else "Unknown"
        Reading <- (srvb2 %>% slice(1298:1304))$Reading[index]
        Reading <- if (!is.na(Reading)) Reading else "Unknown"
        tagList(
          div(style = list(fontSize = 30, fontWeight = 700), value),
          div(style = list(fontSize = 13), Reading),
...
reactable(
  srvb2 %>% slice(1341:1351),
  columns = list(
    Verbs = colDef(
      # Show species under character names
      cell = function(value, index) {
        Meaning <- (srvb2 %>% slice(1341:1351))$Meaning[index]
        Meaning <- if (!is.na(Meaning)) Meaning else "Unknown"
        Reading <- (srvb2 %>% slice(1341:1351))$Reading[index]
        Reading <- if (!is.na(Reading)) Reading else "Unknown"
        tagList(
          div(style = list(fontSize = 30, fontWeight = 700), value),
          div(style = list(fontSize = 13), Reading),
  ...
  columns = list(
      ...
      cell = function(value, index) {
        ...
        tagList(
          ...
          div("•"),
          div(style = list(fontSize = 20, fontWeight = 500, fontFamily = "Rounded Mplus 1c" ), Convo1),
          div(style = list(fontSize = 20, fontWeight = 500, fontFamily = "Rounded Mplus 1c" ), Convo2),
          div(style = list(fontSize = 20, fontWeight = 500, fontFamily = "Rounded Mplus 1c" ), Convo3),
          div(style = list(fontSize = 20, fontWeight = 500, fontFamily = "Rounded Mplus 1c" ), Convo4),
          div(style = list(fontSize = 20, fontWeight = 500, fontFamily = "Rounded Mplus 1c" ), Convo5),
          div(style = list(fontSize = 20, fontWeight = 500, fontFamily = "Rounded Mplus 1c" ), Convo6),
          div(style = list(fontSize = 20, fontWeight = 500, fontFamily = "Rounded Mplus 1c" ), Convo7),
          div(style = list(fontSize = 20, fontWeight = 500, fontFamily = "Rounded Mplus 1c" ), Convo8),
          div(style = list(fontSize = 20, fontWeight = 500, fontFamily = "Rounded Mplus 1c" ), Convo9),
        )
      },
      align = "left", minWidth = 400
      ),
...
reactable(na.omit(rdngr), showPageSizeOptions = FALSE,
    ...
    theme = reactableTheme(
    cellStyle = list(display = "flex", flexDirection = "column", justifyContent = "center", fontSize = "18px", fontWeight = 500),
  ...
  columns = list(
      ...
      cell = function(value, index) {
        ...
        tagList(
          ...
          div(style = list(fontSize = 22, fontWeight = 500), "🆅 ", s2),
          div(style = list(fontSize = 22, fontWeight = 500), "🅅 ", s3),
          div(style = list(fontSize = 22, fontWeight = 500), "🆅 ", s4),
          div(style = list(fontSize = 22, fontWeight = 500), "🅅 ", s5),
          div(style = list(fontSize = 22, fontWeight = 500), "🆅 ", s6),
          div(style = list(fontSize = 22, fontWeight = 500), "🅅 ", s7),
          div(style = list(fontSize = 22, fontWeight = 500), "🆅 ", s8),
          div(style = list(fontSize = 22, fontWeight = 500), "🅅 ", s9),
          div(style = list(fontSize = 22, fontWeight = 500), "🆅 ", s10)
        )
      },
      align = "left", minWidth = 400
      ),
...
<p style="font-size:17px; font-family:Rounded Mplus 1c; font-weight: 400;line-height: 1.5">今週、近くのスーパーがセール中だったので、友達と一緒に行きました。ほとんどの品物が三割引きで、肉や魚半額でした。私の家は家族が多いので、安く買えると嬉しいです。特に冷凍食品はお弁当の時に便利なので、安い時に必ず買います。</p> 
...
<p style="font-size:17px; font-family:Rounded Mplus 1c; font-weight: 400;line-height: 1.5">来月、会社の先輩の結婚式に行く。ちょうどいい服がないので、デパートへ買い物に行った。でも、スーツやドレスなどをいろいろ着てみても、全然決められない。ショッピングは楽しいけれど、少し疲れる。四季まであと一ヶ月あるから、ゆっくり探そう。</p> 
...
<p style="font-size:17px; font-family:Rounded Mplus 1c; font-weight: 400;line-height: 1.5">となりの娘さんは結婚していますが、近くに住んでいるので、よく隣遊び来ています。そして、いつも私の家の駐車場のまいに車を止めて、挨拶もしません。
短い時間ならかまいませんが、長い間止めていると、用事があって出かけたくても、車が出せません。とても困ります。</p> 
<p style="font-size:17px; font-family:Rounded Mplus 1c; font-weight: 400;line-height: 1.5">どうして怒っているのか、わけは知りませんが、あまり大きな声でさわぐとあなたのほうが変な人と思われてしまいます。もう少し静かに話したほうがいいですよ。</p> 
...
<p style="font-size:17px; font-family:Rounded Mplus 1c; font-weight: 400;line-height: 1.5">セナ：もしもし、セナですが。</p> 
<p style="font-size:17px; font-family:Rounded Mplus 1c; font-weight: 400;line-height: 1.5">木村：あ、セナさん、こんにちは。いつもお世話になっています。</p> 
<p style="font-size:17px; font-family:Rounded Mplus 1c; font-weight: 400;line-height: 1.5">セナ：さっき子どもが学校から手紙を持ってきました。でも、漢字が多くて、よくわからないんです。教えていただきたいんですが..。</p> 
<p style="font-size:17px; font-family:Rounded Mplus 1c; font-weight: 400;line-height: 1.5">木村：手紙ですね。ちっと待ってください。えーと、来週の日曜日に、特別な授業があります。お母さんやお父さんに見てもらいます。日曜日に行うので、なるべく出席してください。かわりに、月曜日が休みになるそうですよ。</p> 
<p style="font-size:17px; font-family:Rounded Mplus 1c; font-weight: 400;line-height: 1.5">木村：困ったがったら、遠慮しないで、何でも相談してくださいね。</p> 
<p style="font-size:17px; font-family:Rounded Mplus 1c; font-weight: 400;line-height: 1.5">セナ：どうもありがとうございます。</p> 
...
Kimura: It's a letter. Please wait a minute. Let's see, next Sunday, there will be a special class. I would to have the parents to watch the class. Since it will be held on a Sunday, please try to attend. In exchange for it, Monday will be a day off.  
...
3.1 **という**`describe something (external relative clause)`**⚭　Plain-From + toiu + N**

    ・最近は大学卒業しても就職が難しいという話を聞きました
    ・I've heard that it's difficult to find a job after graduating from college these days

4.1 **だろうと思う**`I guess/think that it seems/probably`**⚭　Plain-From + darou + to omou **
...
8.1 **ていく**`go after doing・do go・go away・to keep(describe temporal and spatial changes/action)`**⚭　Vte + iku**

    ・冬になると渡り鳥は南の方へ飛んでいく  
    ・In the winter, migratory birds fly south

8.2 **てくる**`come back after doing・do come・come in・to continue(describe temporal, spatial & perceptional changes)`**⚭　Vte + kuru**
